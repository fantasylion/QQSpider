...
class BlogSpider(object):
    ...
    def beginer(self):
        ...
        if blog_list:
            ...
            myBlog = pool.map(self.get_blog, zip(list(blog_list.keys()), list(blog_list.values())))
            pool.close()
            pool.join()
            ...
            try:
                ...
                if r.status_code == 403:
                    ...
                    if blog == None:  # 如果没有获取到内容
                        ...
                        myBlog["isTransfered"] = True if '[转]' in soup.text else False  # 判断日志是否属于转载
